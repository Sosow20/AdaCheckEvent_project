...
export default function NavBar({ search, setSearch }) {
    ...
    return (
        <>
            <div className="navBar bg-linear-to-b from-[#38b0ec] flex  sm:flex-row sm:items-center sm:justify-between gap-4 p-4 lg:h-lg">
                <h1 className=" logo font-bold text-white lg:text-3xl ">ADA CHECK<span className="text-[#ff9e27]">EVENT</span></h1>
                <div className="searchBox sm:flex gap-2">
                    <input value={search} type="text" placeholder="Rechercher événement ..." onChange={handleOnChange} className="bg-white border-2 border-black p-1 rounded-sm lg:w-lg" />
                </div>
                <div className="btnMode flex items-center justify-end w-full sm:w-auto gap-3">
                    <svg viewBox="0 0 16 16" height="3em" width="3em" xmlns="http://www.w3.org/2000/svg" className="cursor-pointer fill-[#ff9e27]">
                        <path fill-rule="evenodd" d="M3 3a2 2 0 012-2h6a2 2 0 012 2v12l-5-3-5 3V3z" clip-rule="evenodd"></path>
                    </svg>
                    <button className="switchMode bg-[#1B3238] size-15 rounded-4xl cursor-pointer flex place-content-center items-center" onClick={() => setMode(!mode)}>
                        {mode ?
                            <svg viewBox="0 0 512 512" height="3em" width="3em" className=" fill-[#38B0EC] duration-500">
